...
(function () {
    ...
    config.wallets.forEach((wallet, index) => {
      ...
      card.innerHTML =
        '<div class="wallet-header">' +
          '<img src="' + esc(wallet.logo) + '" alt="' + esc(wallet.name) + ' Logo" class="crypto-logo" loading="lazy">' +
          '<h3>' + esc(wallet.name) + '</h3>' +
        '</div>' +
        '<div class="address-bar">' +
          '<span class="address-text">' + esc(wallet.address) + '</span>' +
          '<div class="address-actions">' +
            '<button class="qr-btn" data-address="' + esc(wallet.address) + '">' + t('donate.qr_button') + '</button>' +
            '<button class="copy-btn" data-address="' + esc(wallet.address) + '">' + t('donate.copy_button') + '</button>' +
          '</div>' +
        '</div>';
  ...
  function renderRialOptions() {
    ...
    config.rialOptions.forEach((option, index) => {
      ...
      const card = document.createElement('div');
      card.className = 'rial-card' + (option.featured ? ' featured' : '');
      card.style.animationDelay = (index * 0.08) + 's';
      const featuredBadge = option.featured ? '<span class="rial-badge">' + t('donate.rial_popular') + '</span>' : '';
      card.innerHTML =
        '<div class="rial-header">' +
          '<span class="rial-amount">' + esc(amt) + '</span>' +
          featuredBadge +
        '</div>' +
        '<div class="rial-bar">' +
          '<span class="rial-value">' + esc(amt) + '</span>' +
          '<a href="' + esc(option.url) + '" target="_blank" rel="noopener" class="rial-btn">' + t('donate.rial_button') + '</a>' +
        '</div>';
  ...
  function showQR(address) {
    if (!dom.qrModal || !dom.qrCanvas || typeof QRious === 'undefined') return;
    dom.qrCanvas.width = 220;
    dom.qrCanvas.height = 220;
    new QRious({ element: dom.qrCanvas, value: address, size: 220, background: '#ffffff', foreground: '#05070f', level: 'M' });
  ...
  function hideQR() {
    ...
    if (lastFocusedBeforeQr) lastFocusedBeforeQr.focus();
  }
  ...
  function copyToClipboard(address, button) {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(address).then(done).catch(() => fallbackCopy(address, button, done, resetText));
    } else {
      fallbackCopy(address, button, done, resetText);
    }
  }

  function fallbackCopy(address, button, done, resetText) {
    try {
  ...
  function init() {
    ...
    document.addEventListener('langchange', updateDonateTexts);
  }
